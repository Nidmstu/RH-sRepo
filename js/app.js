...
function populateCourseDropdown() {
  ...
  // Get visible courses (not hidden)
  ...
  // Set the current course if it exists
...
function populateDays(courseId) {
  ...
  // If no course selected, return
  if (!courseId) return;
  
  // Switch course in courseManager
  window.courseManager.switchProfession(courseId);
  
  // Get days for the selected course
  ...
  // Also populate vocabulary options
...
function populateVocabulary(courseId) {
  ...
  // Clear existing vocabulary buttons
  vocabularyContainer.innerHTML = '';
  
  // Get special lessons for the selected course
  const specialLessons = window.courseManager.getSpecialLessons() || [];
  
  // Add buttons for special lessons
  ...
  // If no special lessons, show a message
...
function handleProfessionChange() {
  ...
  // Check if course has a redirect URL
  ...
  // Otherwise populate days for the selected course
...
function openSpecialLesson(lessonId) {
  ...
  // Setup currentTopic for compatibility with existing code
  window.currentTopic = {
  ...
  // Hide day selection and show guide
  ...
  // Hide test button if no test is available
  const testButton = document.getElementById('test-button');
  if (testButton) {
    if (lesson.testSource && lesson.testSource.url) {
      testButton.classList.remove('hidden');
...
async function loadLessonContent() {
  ...
  try {
    // Get content from courseManager
...
function goBackToTaskSelection() {
  ...
  // If a day was selected before, show task selection
...
function openVocabulary() {
  // Find vocabulary in special lessons
  const specialLessons = window.courseManager.getSpecialLessons() || [];
