...
function renderProfessionsList() {
  ...
  // Получаем список направлений из CourseManager (исключая скрытые)
  const professions = courseManager.getProfessions(false);

  // Очищаем список
  professionsList.innerHTML = '';

  // Создаем элементы списка для каждой профессии
  professions.forEach(professionId => {
...
// Функция для обновления списка дней с учетом скрытия курсов
function updateDaysList() {
  const daySelectionContainer = document.getElementById('day-selection');
  if (!daySelectionContainer) return;

  // Получаем список дней, учитывая скрытые курсы
  const days = courseManager.getDays();
