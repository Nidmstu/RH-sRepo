...
class DevMode {
  ...
  createDevPanel() {
    ...
    devPanel.innerHTML = `
      <div class="dev-panel-header">
        ...
        <div class="dev-panel-actions">
          ...
          <button id="dev-panel-localStorage" title="Управление localStorage"><i class="fas fa-database"></i></button>
    ...
    document.getElementById('dev-panel-localStorage').addEventListener('click', () => {
  ...
  addDevPanelStyles() {
    ...
    styleElement.textContent = `
      ...
      /* Стили для менеджера localStorage */
      ...
      .localStorage-manager-title {
        color: #61afef;
        font-weight: bold;
        margin: 0;
      }
      
      .localStorage-manager-actions {
        display: flex;
        gap: 5px;
      }
      ...
      .localStorage-item-actions {
        display: flex;
        gap: 5px;
      }
      
      .localStorage-item-content {
        padding: 8px;
      ...
      .localStorage-item-content.visible {
        display: block;
      }
      
      .localStorage-value-editor {
        width: 100%;
        min-height: 100px;
        background-color: #1e2127;
        color: #abb2bf;
  ...
  /**
   * Показать менеджер localStorage
   */
  showLocalStorageManager() {
    // Создаем контейнер для менеджера localStorage
    const localStorageManager = document.createElement('div');
    localStorageManager.className = 'localStorage-manager';
    
    // Заголовок и кнопки действий
    const header = document.createElement('div');
    header.className = 'localStorage-manager-header';
    header.innerHTML = `
      <h4 class="localStorage-manager-title">Менеджер localStorage</h4>
      <div class="localStorage-manager-actions">
        <button class="dev-log-button" id="refresh-localStorage">Обновить</button>
        <button class="dev-log-button" id="clear-localStorage" style="background-color: rgba(224, 108, 117, 0.2);">Очистить всё</button>
        <button class="dev-log-button" id="add-localStorage-item">Добавить</button>
      </div>
    `;
    
    // Добавляем информацию о размере localStorage
    const storageInfo = document.createElement('div');
    storageInfo.style.marginBottom = '10px';
    storageInfo.style.color = '#aaa';
    storageInfo.style.fontSize = '12px';
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      totalSize += (key.length + value.length) * 2; // В байтах (UTF-16)
    }
    ...
    if (totalSize < 1024) {
      ...
      sizeString = `${(totalSize / 1024).toFixed(2)} KB`;
    } else {
      sizeString = `${(totalSize / (1024 * 1024)).toFixed(2)} MB`;
    }
    
    storageInfo.textContent = `Использовано: ${sizeString} из 5MB (${(totalSize / (5 * 1024 * 1024) * 100).toFixed(2)}%)`;
    
    // Список элементов localStorage
    const itemsList = document.createElement('div');
    itemsList.className = 'localStorage-items-list';
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      const itemSize = (key.length + value.length) * 2; // В байтах (UTF-16)
      let itemSizeString = '';
      if (itemSize < 1024) {
        itemSizeString = `${itemSize} B`;
      } else if (itemSize < 1024 * 1024) {
        itemSizeString = `${(itemSize / 1024).toFixed(2)} KB`;
      } else {
        itemSizeString = `${(itemSize / (1024 * 1024)).toFixed(2)} MB`;
      }
      
      const item = document.createElement('div');
      item.className = 'localStorage-item';
      item.innerHTML = `
        <div class="localStorage-item-header" data-key="${key}">
          <span class="localStorage-item-key">${key}</span>
          <div style="display: flex; align-items: center; gap: 10px;">
            <span class="localStorage-item-size">${itemSizeString}</span>
            <div class="localStorage-item-actions">
              <button class="dev-log-button edit-localStorage-btn" data-key="${key}">Редактировать</button>
              <button class="dev-log-button" style="background-color: rgba(224, 108, 117, 0.2);" data-key="${key}">Удалить</button>
            </div>
          </div>
        </div>
        <div class="localStorage-item-content" id="localStorage-content-${this.sanitizeId(key)}">
          <div class="localStorage-value-preview"></div>
        </div>
      `;
    ...
    localStorageManager.appendChild(header);
    localStorageManager.appendChild(storageInfo);
    localStorageManager.appendChild(itemsList);
    
    // Добавляем в панель разработчика
    const logsContainer = document.getElementById('dev-panel-logs');
    if (logsContainer) {
      // Удаляем предыдущий менеджер, если он уже есть
      const existingManager = logsContainer.querySelector('.localStorage-manager');
      if (existingManager) {
        existingManager.remove();
      }
      
      logsContainer.prepend(localStorageManager);
      
      // Обработчики событий для элементов localStorage
      const itemHeaders = logsContainer.querySelectorAll('.localStorage-item-header');
      itemHeaders.forEach(header => {
        header.addEventListener('click', (e) => {
          if (!e.target.classList.contains('dev-log-button')) {
            const key = header.getAttribute('data-key');
            const content = document.getElementById(`localStorage-content-${this.sanitizeId(key)}`);
            
            // Показываем или скрываем содержимое
            content.classList.toggle('visible');
            
            // Если видимо, то заполняем превью
            if (content.classList.contains('visible')) {
              const value = localStorage.getItem(key);
              const preview = content.querySelector('.localStorage-value-preview');
              
              try {
                // Пытаемся отформатировать как JSON
                const parsedValue = JSON.parse(value);
                preview.innerHTML = this.formatJson(parsedValue);
              } catch (e) {
      ...
      const deleteButtons = logsContainer.querySelectorAll('.localStorage-item-actions button:last-child');
      deleteButtons.forEach(button => {
        button.addEventListener('click', (e) => {
          e.stopPropagation();
          const key = button.getAttribute('data-key');
          
          if (confirm(`Удалить элемент '${key}' из localStorage?`)) {
            localStorage.removeItem(key);
            this.logMessage(`Элемент '${key}' удален из localStorage`, 'info');
            this.showLocalStorageManager(); // Обновляем список
          }
        });
      });
      
      // Обработчики для кнопок редактирования
      const editButtons = logsContainer.querySelectorAll('.edit-localStorage-btn');
      editButtons.forEach(button => {
        button.addEventListener('click', (e) => {
          ...
          // Проверяем, есть ли уже редактор
          let editor = content.querySelector('.localStorage-value-editor');
          let buttonsContainer = content.querySelector('.localStorage-item-buttons');
          
          if (!editor) {
            // Создаем текстовый редактор
            editor = document.createElement('textarea');
            editor.className = 'localStorage-value-editor';
            editor.value = value;
            
            // Кнопки для сохранения/отмены
            buttonsContainer = document.createElement('div');
            buttonsContainer.className = 'localStorage-item-buttons';
            buttonsContainer.innerHTML = `
              <button class="dev-log-button save-localStorage-btn" data-key="${key}">Сохранить</button>
              <button class="dev-log-button cancel-localStorage-btn" data-key="${key}">Отмена</button>
            `;
            ...
            const saveButton = buttonsContainer.querySelector('.save-localStorage-btn');
            saveButton.addEventListener('click', () => {
              try {
                // Проверяем, можно ли распарсить как JSON (если это JSON)
                try {
                  JSON.parse(editor.value);
                } catch (e) {
                  if (confirm('Значение не является валидным JSON. Сохранить как строку?')) {
                ...
                this.logMessage(`Элемент '${key}' обновлен в localStorage`, 'success');
                this.showLocalStorageManager(); // Обновляем список
              } catch (error) {
                this.logMessage(`Ошибка при сохранении: ${error.message}`, 'error');
              }
            });
            
            const cancelButton = buttonsContainer.querySelector('.cancel-localStorage-btn');
      ...
      const refreshButton = document.getElementById('refresh-localStorage');
      refreshButton.addEventListener('click', () => {
        this.showLocalStorageManager();
        this.logMessage('Список localStorage обновлен', 'info');
      });
      
      // Обработчик для кнопки очистки всего localStorage
      const clearButton = document.getElementById('clear-localStorage');
      clearButton.addEventListener('click', () => {
        if (confirm('Вы точно хотите полностью очистить localStorage? Это удалит все данные приложения.')) {
          localStorage.clear();
          this.logMessage('localStorage полностью очищен', 'warning');
          this.showLocalStorageManager();
        }
      });
      
      // Обработчик для кнопки добавления нового элемента
      const addButton = document.getElementById('add-localStorage-item');
      addButton.addEventListener('click', () => {
        const newKey = prompt('Введите ключ для нового элемента:');
        if (newKey) {
          if (localStorage.getItem(newKey) !== null) {
            if (!confirm(`Элемент с ключом '${newKey}' уже существует. Заменить?`)) {
              return;
            }
          }
          ...
          if (newValue !== null) {
            ...
            this.logMessage(`Добавлен новый элемент с ключом '${newKey}'`, 'success');
            this.showLocalStorageManager();
          }
        }
      });
  ...
  /**
   * Вспомогательная функция для санитизации ID
   */
  sanitizeId(str) {
